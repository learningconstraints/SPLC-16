...
oracle_contrasts_with_ref::oracle_contrasts_with_ref()
{
    //ctor
}

oracle_contrasts_with_ref::oracle_contrasts_with_ref(string fname, string fname_ref, int f)
:oracle_contrasts(fname,f),filename_reference(fname_ref),vid_reference(filename_reference)
{
...
bool oracle_contrasts_with_ref::decide()
{
    ...
    //result of every computed frame (ourier transform)
    /// ! Initialization ?
    Mat tmp, tmp_ref;
...
Mat oracle_contrasts_with_ref::compute_fourier_transform(Mat& img)
{
    Mat tmp_img;
...
*/
void oracle_contrasts_with_ref::display_fourier_transform(Mat& fourierImg)
{
    ...
    //waitKey(0);*/
    oracle_contrasts::display_fourier_transform(fourierImg);
...
*/
void oracle_contrasts_with_ref::rearrange_fourier_coeff(Mat& fourierImg)
{
    ...
    q2.copyTo(q1);
    tmp.copyTo(q2);*/
    oracle_contrasts::rearrange_fourier_coeff(fourierImg);
}
...
*/
void oracle_contrasts_with_ref::compute_rings(const Mat& fourier_coeff, vector<double>& cumul_distrib, const int min_rings, const int delta)
{
    ...
    oracle_contrasts::compute_rings(fourier_coeff,cumul_distrib);
...
*/
void oracle_contrasts_with_ref::compute_cumulative(const Mat& fourierImg, const int number_of_rings, int nb_rings, Mat& fourierMasked, vector<double>& cumul_distrib, double nb_freq)
{
    ...
    oracle_contrasts::compute_cumulative(fourierImg,number_of_rings,nb_rings,fourierMasked,cumul_distrib,nb_freq);
}

bool oracle_contrasts_with_ref::normalize_distrib(vector<double>& cumul_distrib)
{
    ...
    return isNotZero;*/
    return oracle_contrasts::normalize_distrib(cumul_distrib);
}
...
*/
bool oracle_contrasts_with_ref::display_cumulative(vector<double>& cumul_distrib)
{
    ...
    return oracle_contrasts::display_cumulative(cumul_distrib);
}
...
*/
double oracle_contrasts_with_ref::prepare_decision(vector<double>& cumul_distrib, vector<double>& cumul_distrib_ref)
{
...
*/
double oracle_contrasts_with_ref::compute_value_ref(vector<double>& cumul_distrib, int index)
{
...
bool oracle_contrasts_with_ref::decision(double val)
{
    ///decide whether computable or not
    bool isNotBlurredNorNoisy = (val <= 0.005&& val >= -0.005)? true:false;
